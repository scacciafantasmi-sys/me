...
3. **Rilevamento automatico delle scene**: qualunque video abbastanza lungo (caricato a mano o importato da YouTube, non fa differenza) viene diviso da solo in scene — l'app individua i cambi di scena/inquadratura con lo stesso punteggio di "scene change" di ffmpeg, che in pratica scatta sui tagli netti e sui movimenti/cambi rapidi di inquadratura — e tratta ogni scena come una sorgente a sé nel montaggio. Per le clip un po' più corte (ma non cortissime) la divisione non parte da sola ma resta disponibile a comando con un pulsante ✂️ sulla clip.
